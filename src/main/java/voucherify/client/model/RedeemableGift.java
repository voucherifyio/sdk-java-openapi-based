...
public class RedeemableGift {
  ...
  * @throws IOException if the JSON Element is invalid with respect to RedeemableGift
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RedeemableGift.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in RedeemableGift is not found in the empty JSON string", RedeemableGift.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RedeemableGift.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RedeemableGift` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
