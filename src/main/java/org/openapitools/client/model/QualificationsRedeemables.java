...
import org.openapitools.client.model.QualificationsRedeemable;
...
public class QualificationsRedeemables {
  ...
  * @throws IOException if the JSON Element is invalid with respect to QualificationsRedeemables
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!QualificationsRedeemables.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!QualificationsRedeemables.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `QualificationsRedeemables` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
