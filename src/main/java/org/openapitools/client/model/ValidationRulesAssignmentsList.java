...
import org.openapitools.client.model.BusValRuleAssignment;
...
public class ValidationRulesAssignmentsList {
  ...
  private List<BusValRuleAssignment> data = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to ValidationRulesAssignmentsList
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ValidationRulesAssignmentsList.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ValidationRulesAssignmentsList is not found in the empty JSON string", ValidationRulesAssignmentsList.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ValidationRulesAssignmentsList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ValidationRulesAssignmentsList` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ValidationRulesAssignmentsList.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArraydata.size(); i++) {
        BusValRuleAssignment.validateJsonElement(jsonArraydata.get(i));
      };
  }
